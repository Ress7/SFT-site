...
const hexToNormalizedRGB = hex => {
  const clean = hex ? hex.replace('#', '') : 'ffffff';
...
float cnoise(vec3 P){
  ...
  float n100 = dot(g100, vec3(Pf1.x,Pf1.yz));
...
function createStackedPlanesBufferGeometry(n, width, height, spacing, heightSegments) {
  ...
  const speeds = new Float32Array(numVertices);
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j <= heightSegments; j++) {
      ...
      speeds.set([speedDir, speedDir], vertexOffset);
...
export default function Beams({
  ...
  useEffect(() => {
    ...
    const beamMaterial = extendMaterial(THREE.MeshStandardMaterial, {
      ...
      vertexHeader: `
        ...
        float getPos(vec3 pos) {
          ...
          vec3 noisePos =
            vec3(pos.x * 0., pos.y - uv.y, pos.z + time * uSpeed * 3. * speed) * uScale;
    ...
    // Geometry
    const geometry = createStackedPlanesBufferGeometry(beamNumber, beamWidth, beamHeight, spacing, 100);
    ...
    const animate = () => {
      ...
      beamMaterial.uniforms.time.value += delta * 0.5; // Much smoother and slower time progression
  ...
  }, [beamWidth, beamHeight, beamNumber, lightColor, baseColor, speed, noiseIntensity, scale, rotation, spacing]);
